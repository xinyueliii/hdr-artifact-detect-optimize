...
from models.loss import L1MuLoss, JointReconPerceptualLoss, JointPerceptualLoss
from models.SCTNet import SCTNet
from utils.utils import *
from HADetector.detect import detect_artifact
...
def get_args():
    ...
    parser.add_argument("--dataset_dir", type=str, default='/root/autodl-tmp/SIG17',
                        help='dataset directory'),
    parser.add_argument('--patch_size', type=int, default=128),
    parser.add_argument("--sub_set", type=str, default='sig17_training_crop128_stride64',
                        help='dataset directory')
    parser.add_argument('--logdir', type=str, default='/root/autodl-tmp/Kal_SCT',
...
def train(args, model, device, train_loader, optimizer, epoch, criterion):
    ...
    for batch_idx, batch_data in enumerate(tqdm(train_loader, desc="Training Progress")):
        ...
        # Detect and process mask
        _, masks = detect_artifact(pred)
        for mask_idx in range(len(masks)):
            mask = masks[mask_idx]
            masks[mask_idx] = (mask > 0.5).float()
            mask = masks[mask_idx]
        
        # list -> tensor
        masks = torch.stack(masks)

        loss = criterion(pred, label, masks)
...
def test(args, model, device, optimizer, epoch, cur_psnr, **kwargs):
    ...
    torch.save(save_dict, os.path.join(args.logdir, f'val_checkpoint_epoch_{epoch + 1}.pth'))
    with open(os.path.join(args.logdir, 'log.json'), 'a') as f:
            f.write('best epoch:' + str(epoch) + '\n')
            f.write('Test set: Average PSNR: {:.4f}, PSNR_mu: {:.4f}, SSIM_l: {:.4f}, SSIM_mu: {:.4f}\n'.format(
                psnr_l.avg,
                psnr_mu.avg,
                ssim_l.avg,
                ssim_mu.avg
