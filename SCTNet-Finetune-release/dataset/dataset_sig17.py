...
class Img_Dataset(Dataset):
    def __init__(self, ldr_path, label_path, exposure_path, patch_size, scene_name):
...
def SIG17_Test_Dataset(root_dir, patch_size):
    ...
    for scene in range(len(scenes_list)):
        if scenes_list[scene] == ".ipynb_checkpoints":
            continue
        ...
        scene_names_list += [scenes_list[scene]]
    for ldr_dir, label_dir, expo_times_dir, scene_name in zip(ldr_list, label_list, expo_times_list, scene_names_list):
        yield Img_Dataset(ldr_dir, label_dir, expo_times_dir, patch_size, scene_name)
