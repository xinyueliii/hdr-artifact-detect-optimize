...
def main():
    ...
    parser.add_argument("--data_root", type=str, default='/root/autodl-tmp/data/Kal')
    ...
    global counter
    counter = 0
    all_scenes = sorted([p for p in glob.glob(os.path.join(full_size_training_data_path, '*')) if os.path.isdir(p)])
    ...
    print("Done. Saved {} patches to {}".format(counter, cropped_training_data_path))
