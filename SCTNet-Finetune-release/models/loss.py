...
class JointPerceptualLoss(nn.Module):
    def __init__(self, alpha=0.01, mu=5000, mask_weight=1.0):
        super(JointPerceptualLoss, self).__init__()
        self.alpha = alpha
        self.mu = mu
    ...
    def forward(self, input, target, mask=None):
        ...
        # Compute original losses: l1 loss + vgg loss
        loss_recon = self.loss_recon(input, target)
        loss_vgg = self.loss_vgg(input_mu, target_mu)
        ...
        masked_input_mu = range_compressor(masked_input, self.mu)
        masked_target_mu = range_compressor(masked_target, self.mu)
        
        # Compute masked losses
        masked_loss_recon = self.loss_recon(masked_input, masked_target)

        if mask.sum() == 0:
            masked_loss_recon = torch.tensor(0.0, device=masked_loss_recon.device)
        else:
            masked_loss_recon = masked_loss_recon * mask.numel() / mask.sum()
            
        # Combine losses
        masked_loss = masked_loss_recon * self.mask_weight
        original_loss = loss_recon + self.alpha * loss_vgg


        total_loss = original_loss + self.mask_weight * masked_loss
